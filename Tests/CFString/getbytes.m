...
int main (void)
{
  ...
  n = CFStringGetBytes (eacute, CFRangeMake (0, 1), kCFStringEncodingUTF16BE,
    0, false, buf, sizeof (buf), &used);
  PASS_CF (n == 1 && used == 2 && buf[0] == 0x00 && buf[1] == 0xE9,
    "GetBytes converts U+00E9 to UTF-16BE 00 E9.");

  used = 0;
  n = CFStringGetBytes (eacute, CFRangeMake (0, 1), kCFStringEncodingUTF16LE,
    0, false, buf, sizeof (buf), &used);
  PASS_CF (n == 1 && used == 2 && buf[0] == 0xE9 && buf[1] == 0x00,
    "GetBytes converts U+00E9 to UTF-16LE E9 00.");
  ...
  n = CFStringGetBytes (eacute, CFRangeMake (0, 1), kCFStringEncodingASCII,
    0, false, buf, sizeof (buf), &used);
  PASS_CF (n == 0 && used == 0,
    "GetBytes converts nothing when loss is not allowed.");
